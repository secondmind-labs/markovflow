...
class BaseKalmanFilter(tf.Module, ABC):
    ...
    def forward_filter(self) -> Tuple[tf.Tensor, tf.Tensor, tf.Tensor, tf.Tensor, tf.Tensor]:
        """
        ...
        # [..., state_dim, state_dim]
        P_0 = tf.matmul(
            self.prior_ssm.cholesky_initial_covariance,
            self.prior_ssm.cholesky_initial_covariance,
            transpose_b=True,
        )
        # [..., state_dim]
        ...
        Q_s = tf.matmul(
            self.prior_ssm.cholesky_process_covariances,
            self.prior_ssm.cholesky_process_covariances,
            transpose_b=True,
        )
        ...
        R = tf.linalg.cholesky_solve(
            tf.linalg.cholesky(self._r_inv), tf.eye(self.emission.output_dim, dtype=default_float())
        )
        ...
        def step(
            log_liks, filter_mus, filter_covs, pred_mus, pred_covs, counter
            ...
            filter_mean = pred_mean + tf.linalg.matvec(
                Kt, y_k - tf.linalg.matvec(H_k, pred_mean), transpose_a=True
            )
            filter_cov = pred_cov - tf.matmul(Kt, S, transpose_a=True) @ Kt

            tmp = tf.linalg.triangular_solve(
                chol, (y_k - tf.linalg.matvec(H_k, pred_mean))[..., None]
            )
            log_lik = -0.5 * (
                tf.reduce_sum(tf.square(tmp))
                + np.log(2.0 * np.pi) * self.emission.output_dim
            ...
            return (
                tf.concat([log_liks, log_lik[..., None]], axis=-1),
        ...
        log_liks, filter_mus, filter_covs, pred_mus, pred_covs, _ = tf.while_loop(
    ...
    def backward_filter(self) -> Tuple[tf.Tensor, tf.Tensor, tf.Tensor, tf.Tensor, tf.Tensor]:
        """
        ...
        log_liks, filter_mus, filter_covs, pred_mus, pred_covs, _ = tf.while_loop(
            cond=lambda _, __, ___, ____, _____, counter: counter < num_transitions,
            body=step,
            loop_vars=loop_vars,
        ...
        return (
            tf.reverse(log_liks, axis=[-1]),
            tf.reverse(filter_mus, axis=[-2]),
