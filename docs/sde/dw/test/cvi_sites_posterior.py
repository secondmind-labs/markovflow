...
if __name__ == '__main__':
    ...
    # Only Data sites
    t_vgp_trainer_data_sites = tVGPTrainer(observations, likelihood, t, sde_p, data_sites_lr=0.9, all_sites_lr=0.1,
                                          update_all_sites=False)
    t_vgp_elbo_vals_data_sites, _, _ = t_vgp_trainer_data_sites.run(update_prior=False)
    print(f"t-VGP model (Data-sites) : {t_vgp_elbo_vals_data_sites[-1]}")

    t_vgp_model_data_sites = t_vgp_trainer_data_sites.tvgp_model

    data_sites_nat1 = t_vgp_model_data_sites.data_sites.nat1
    data_sites_nat2 = t_vgp_model_data_sites.data_sites.nat2
    noise_var = likelihood.variance
    np.testing.assert_array_almost_equal(data_sites_nat1.numpy(), observations[1] / noise_var)
    np.testing.assert_array_almost_equal(tf.reduce_sum(data_sites_nat2),
                                         data_sites_nat2.shape[0] * (-1 / (2 * noise_var)))

    m_data_sites, S_data_sites = t_vgp_model_data_sites.dist_q.marginals
    m_data_sites = m_data_sites.numpy().reshape(-1)
    S_data_sites_std = np.sqrt(S_data_sites.numpy()).reshape(-1)
    ...
    plt.fill_between(
        t,
        y1=(m_data_sites.reshape(-1) - 2 * S_data_sites_std.reshape(-1)).reshape(-1, ),
        y2=(m_data_sites.reshape(-1) + 2 * S_data_sites_std.reshape(-1)).reshape(-1, ),
        edgecolor="blue",
        facecolor=(0, 0, 0, 0.)
    )

    plt.plot(t.numpy().reshape(-1), m_all_sites.reshape(-1), color="black", linestyle="dashed", label="t-VGP (All sites)")
    plt.plot(t.numpy().reshape(-1), m_data_sites.reshape(-1), color="blue", alpha=0.5, label="t-VGP (Data sites)")

    plt.legend()
    plt.savefig("test_sites_posterior.png")
    plt.show()
