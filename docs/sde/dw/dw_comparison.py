...
from markovflow.sde.sde import DoubleWellSDE
...
import sys
sys.path.append("..")
from sde_exp_utils import generate_dw_data, get_gpr, predict_vgp, predict_ssm, predict_gpr, plot_observations, plot_posterior, \
...
observation_vals, observation_grid, latent_process, time_grid = generate_dw_data(q=q, x0=x0, t0=t0, t1=t1,
...
kernel = Matern12(lengthscale=1., variance=q)
...
ssm_model.initial_chol_cov = tf.math.sqrt(q).numpy().item() * tf.ones_like(ssm_model.initial_chol_cov)
ssm_model.fx_covs = q + 0 * ssm_model.fx_covs
...
vgp_model = VariationalMarkovGP(input_data=input_data,
                                ...
                                lr=0.01)
vgp_model.p_initial_cov = q * tf.ones((1, 1), dtype=DTYPE)
