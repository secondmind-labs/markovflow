...
def generate_data(decay: float, q: float, x0: float, t0: float, t1: float, noise_var: float,
    ...
    latent_process, time_grid, test_values, test_grid = generate_ou_data(decay=decay, q=q, x0=x0, t0=t0,
                                                                         t1=t1, simulation_dt=dt,
...
if __name__ == '__main__':
    # python ou_generate_data.py -s 128 -d .8 -q 1.2 -n 10 -v 0.5 -t0 0. -t1 5. -dt 0.01 -x0 1.
    ...
    parser.add_argument('-d', '--decay', type=float,
                        help="Decay value for the OU SDE drift (without the negative sign).", required=True)
    ...
    observation_vals, observation_grid, \
    latent_process, time_grid, test_values, test_grid = generate_data(args.decay, args.q, args.x0, args.t0,
